<x-layouts.app :title="'Editar Servicio'">
    <div class="max-w-3xl mx-auto p-6">
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-white mb-6">
            <i class="fas fa-edit text-orange-500 mr-2"></i>Editar Servicio
        </h2>
        ...
        <form action="{{ route('servicios.update', $servicio->servicios_id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="3"
                          class="w-full mt-1 px-4 py-2 border-2 rounded focus:ring-orange-500 focus:border-orange-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white">{{ old('descripcion', $servicio->descripcion) }}</textarea>
            </div>

            <div>
                <label for="precio" class="block font-medium text-gray-700 dark:text-gray-300">Precio (S/)</label>
                <input type="number" step="0.01" id="precio" name="precio" value="{{ old('precio', $servicio->precio) }}"
                       class="w-full mt-1 px-4 py-2 border-2 rounded focus:ring-orange-500 focus:border-orange-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white" required>
            </div>

            <div>
                <label for="capacidad_maxima" class="block font-medium text-gray-700 dark:text-gray-300">Capacidad máxima</label>
                <input type="number" id="capacidad_maxima" name="capacidad_maxima" value="{{ old('capacidad_maxima', $servicio->capacidad_maxima) }}"
                       class="w-full mt-1 px-4 py-2 border-2 rounded focus:ring-orange-500 focus:border-orange-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white" required>
            </div>

            <div>
                <label for="duracion_servicio" class="block font-medium text-gray-700 dark:text-gray-300">Duración (minutos)</label>
                <input type="number" id="duracion_servicio" name="duracion_servicio" value="{{ old('duracion_servicio', $servicio->duracion_servicio) }}"
            ...
            <div>
                <label for="imagen_destacada" class="block font-medium text-gray-700 dark:text-gray-300">URL de imagen destacada</label>
                <input type="text" id="imagen_destacada" name="imagen_destacada" value="{{ old('imagen_destacada', $servicio->imagen_destacada) }}"
                       class="w-full mt-1 px-4 py-2 border-2 rounded focus:ring-orange-500 focus:border-orange-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white">
            </div>
            ...
            <div>
                ...
                <select id="categorias_servicios_id" name="categorias_servicios_id" required
                    ...
                    @foreach ($categorias as $categoria)
                        <option value="{{ $categoria->categorias_servicios_id }}"
                            {{ old('categorias_servicios_id', $servicio->categorias_servicios_id) == $categoria->categorias_servicios_id ? 'selected' : '' }}>
                            {{ $categoria->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex justify-between">
                ...
                <button type="submit"
                        class="px-4 py-2 bg-orange-500 text-white rounded hover:bg-orange-600">Actualizar</button>
            </div>
        </form>
    </div>
</x-layouts.app>
